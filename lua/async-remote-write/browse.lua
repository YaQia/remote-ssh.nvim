...
local function cache_set(cache_type, key, data)
    ...
    utils.log(
        "Cache SET for " .. cache_type .. ": " .. key .. " (" .. #data .. " items)",
...
function M.get_cache_stats()
    local hit_rate = cache.stats.total_requests > 0 and (cache.stats.hits / cache.stats.total_requests * 100) or 0
...
function M.start_cache_warming(url, options)
    ...
    utils.log(
        "Starting background cache warming for: " .. url .. " (max depth: " .. max_depth .. ")",
        vim.log.levels.DEBUG,
        false,
        config.config
    )
                    ...
                    if cache_warming.stats.directories_warmed % 10 == 0 then
                        utils.log(
                            "Cache warming progress: "
                                .. cache_warming.stats.directories_warmed
                                .. " directories, depth "
                                .. job.current_depth
                                .. "/"
                                .. job.max_depth,
...
function M.finish_cache_warming(warming_key)
    ...
    utils.log(
        "Cache warming completed for: "
            .. job.url
            .. " ("
            .. stats.directories_warmed
            .. " dirs, "
            .. stats.files_cached
            .. " files, "
    ...
    local job_id = vim.fn.jobstart(cmd, {
        ...
        on_exit = function(_, exit_code)
            if exit_code ~= 0 then
                vim.schedule(function()
                    utils.log(
                        "Error listing directory: " .. table.concat(stderr_output, "\n"),
    ...
    local check_job_id = vim.fn.jobstart(check_dir_cmd, {
        ...
        on_exit = function(_, exit_code)
            if exit_code ~= 0 then
                ...
                vim.schedule(function()
                    utils.log(
                        "Error accessing directory: " .. table.concat(check_output, "\n"),
                        vim.log.levels.ERROR,
                        true,
                        config.config
                    )
            ...
            vim.schedule(function()
                ...
                local job_id = vim.fn.jobstart(cmd, {
                    ...
                    on_exit = function(_, exit_code)
                        ...
                        vim.schedule(function()
                            utils.log(
                                "SSH command completed, processing results...",
                                vim.log.levels.INFO,
                                true,
                                config.config
                            )
                            ...
                            for _, file_path in ipairs(output) do
                                ...
                                -- Only process if we haven't seen this path
                                if not seen_paths[rel_path] then
                                    -- Mark as seen
                            ...
                            utils.log(
                                "Found " .. #files .. " files" .. (truncated and " (results limited)" or ""),
                                vim.log.levels.INFO,
                                true,
                                config.config
                            )
    ...
    local job_id = vim.fn.jobstart(cmd, {
        ...
        on_exit = function(_, exit_code)
            ...
            if exit_code ~= 0 then
                vim.schedule(function()
                    utils.log(
                        "Error loading directories: " .. table.concat(stderr_output, "\n"),
                        vim.log.levels.ERROR,
                        true,
                        config.config
                    )
            ...
            vim.schedule(function()
                ...
                utils.log(
                    "Loaded " .. #directories .. " directories" .. (parent_url and " (+ parent)" or ""),
                    vim.log.levels.INFO,
                    true,
                    config.config
                )
    ...
    local job_id = vim.fn.jobstart(cmd, {
        ...
        on_exit = function(_, exit_code)
            ...
            if exit_code ~= 0 then
                vim.schedule(function()
                    utils.log(
                        "Error loading file chunk: " .. table.concat(stderr_output, "\n"),
                        vim.log.levels.ERROR,
                        true,
                        config.config
                    )
            ...
            vim.schedule(function()
                ...
                utils.log(
                    "Loaded " .. #chunk_files .. " files (total: " .. state.total_loaded .. " items)",
                    vim.log.levels.INFO,
                    true,
                    config.config
                )
    ...
    pickers
        .new({}, {
            prompt_title = "Remote Files: "
                .. base_url
                .. " (Tab:cycle status, d:delete, n:new, <C-o>:process, <C-x>:clear)",
                    ...
                    return {
                        ...
                        display = function()
                            ...
                            local highlights = {
                                {
                                    { 0, #prefix },
                                    is_selected and "diffAdded" or (is_to_delete and "diffRemoved" or "Comment"),
                                },
                        ...
                        if confirm == 1 then
                            ...
                            for url, file in pairs(files_to_delete) do
                                ...
                                if remote_info then
                                    ...
                                    if job_id <= 0 then
                                        utils.log(
                                            "Failed to start deletion job for: " .. file.name,
                                            vim.log.levels.ERROR,
                                            true,
                                            config.config
                                        )
                    ...
                    if create_count > 0 then
                        ...
                        for url, file in pairs(files_to_create) do
                            ...
                            if remote_info then
                                ...
                                if job_id <= 0 then
                                    utils.log(
                                        "Failed to start creation job for: " .. file.name,
                                        vim.log.levels.ERROR,
                                        true,
                                        config.config
                                    )
...
function M.show_files_in_telescope(files, base_url, custom_title)
    ...
    pickers
        .new({}, {
            prompt_title = custom_title
                or ("Remote Files: " .. base_url .. " (Tab:cycle status, d:delete, n:new, <C-o>:process, <C-x>:clear)"),
            finder = finders.new_table({
                results = files,
                entry_maker = function(entry)
                    local icon, icon_hl
        ...
        .new({}, {
            ...
            attach_mappings = function(prompt_bufnr, map)
                ...
                local toggle_selection = function()
                    ...
                    elseif selection and selection.value.is_dir then
                        utils.log(
                            "Cannot select directories (press Enter to navigate)",
                            vim.log.levels.INFO,
                            true,
                            config.config
                        )
                ...
                local load_more = function()
                    utils.log(
                        "Load more requested. State: has_more_files="
                            .. tostring(state.has_more_files)
                            .. ", loading="
                    ...
                    if state.has_more_files and not state.loading then
                        ...
                        if state.loading then
                            ...
                            utils.log(
                                "All files have been loaded (total: " .. state.total_loaded .. " items)",
                                vim.log.levels.INFO,
                                true,
                                config.config
                            )
...
function M.show_level_based_telescope(items, base_url)
    ...
    local title = "Level Browse: "
        .. base_url
        .. " ["
        .. file_count
        .. " files, "
        .. dir_count
        .. " dirs] (Enter:navigate, Tab:select, <C-r>:recursive search)"
